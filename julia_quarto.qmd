...
## 5. Final Model Selection & Fine-Tuning
...
ggplot(mapping = aes(x = 1:length(pve), y = cumsum(pve))) + 
  ...
  geom_point(mapping = aes(x = p90, y = cumsum(pve)[p90])) + 
  geom_text(mapping = aes(x = p90, y = cumsum(pve)[p90], 
                          label = paste("(", p90, ", ", round(cumsum(pve)[p90], 3), ")", sep = "")), 
            vjust = 1.5, hjust = 0)
...
# train_data_x_pca$x[, 1:p90]

test_data_x_pca = scale(test_data_x) %*% train_data_x_pca$rotation[, 1:p90]

knn_eval_pca = evaluate_KNN(train_x = train_data_x_pca$x[, 1:p90], train_y = train_data_y, 
